...
public class ColorArcProgressBar extends View {
    ...
    private int mWidth;
    private int mHeight;
    private int diameter = dipToPx(110);  //直径
    ...
    private String bgArcColor = "#CECECE";
    private String dotColor = "#187A74";
...
//        diameter = 9 * getScreenWidth() / 20;
        diameter = dipToPx(168);
        //弧形的矩阵区域
        bgRect = new RectF();
        ...
        vTextPaint.setColor(Color.parseColor("#55545A"));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //起点的圆
        canvas.drawCircle(centerX, bgRect.top, dipToPx((float) 2.5), dotPaint);

        if (isNeedContent) {
            canvas.drawText(String.format("%.0f", curValues), centerX, centerY + textSize + textSize / 3, vTextPaint);
        }
        if (isNeedUnit) {
